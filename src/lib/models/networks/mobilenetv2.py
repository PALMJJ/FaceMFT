...
class MobileNetSeg(nn.Module):
    def __init__(self, base_name,heads,head_conv=24, pretrained = True):
        ...
        for head in self.heads:
            ...
            if heads_conv > 0:
                fc = nn.Sequential(
                  nn.Conv2d(64, heads_conv,
                    kernel_size=3, padding=1, bias=True),
                  nn.ReLU(inplace=True),
                  nn.Conv2d(heads_conv, classes, 
    ...
    def forward(self, x):
        x = self.base(x)
        x = self.dla_up(x)
        # print("===", x.shape)   torch.Size([16, 64, 120, 160])
        ret = {}
        for head in self.heads:
            ret[head] = self.__getattr__(head)(x)
        # print("heads out===>", ret['hm'].shape, ret['wh'].shape, ret['id'].shape, ret['reg'].shape)
        return [ret]
...
def mobilenetv2_10(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load("/home/hengyuli/FaceMFT/src/lib/models/networks/mobilenet_v2-b0353104.pth")
